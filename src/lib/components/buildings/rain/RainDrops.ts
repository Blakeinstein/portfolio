import { chance, createCanvas, getContext, loadImage, random, times } from './utils';
...
export default class RainRenderer {
	dropColor: HTMLImageElement | undefined;
	dropAlpha: HTMLImageElement | undefined;
	...
	constructor(props: { width: number; height: number; scale: number }) {
		...
		this.canvas = createCanvas(props.width, props.height);
		...
		this.dropletsCtx = getContext(this.dropletCanvas);
	...
	drawDroplet(x: number, y: number, r: number) {
		this.drawDrop(
			this.dropletsCtx,
			Object.assign(Object.create(DEFAULT_DROP), {
	...
	renderDropsGfx() {
		...
		const dropBufferCtx = getContext(dropBuffer);
		this.dropsGfx = [...Array(255)].map((_cur, i) => {
			const drop = createCanvas(DROP_SIZE, DROP_SIZE);
			const dropCtx = getContext(drop);
		...
		const clearDropletsCtx = getContext(this.clearDropletsGfx);
	...
	createDrop(options: Partial<Drop>): Drop | null {
		...
		return Object.assign(Object.create(DEFAULT_DROP), options);
	}
	...
	updateDroplets(timeScale: number) {
		...
		this.ctx.drawImage(this.dropletCanvas, 0, 0, this.width, this.height);
	}
