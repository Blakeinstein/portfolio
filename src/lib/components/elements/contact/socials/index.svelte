  ...
  const socials: Record<string, SocialInfo[]> = {
    ...
    "Productivity": [
      ...
      },
      {
        name: "Polywork",
        custom: "polywork",
        icon: `<svg xmlns="http://www.w3.org/2000/svg" class="h-full w-full" viewBox="0 0 37 37" fill="none">
  ...
  <ul class="flex justify-evenly content-evenly w-full h-full" >
    {#each Object.entries(socials) as [group, links]}
      <li class="flex flex-col flex-nowrap place-items-center w-1/2 goo">
        <span class="bg-black rounded-full p-2 h-32 w-32 text-white grid place-items-center">{group}</span>

          {#each links as social}
            <Link
